# Pass 7.38.32 — MC02 static icon alignment

- Restored Velocity Vault objective copy to the previous one-line version: `Capture the engineering data needed for Santa-1.`
- Removed all MC02 diagnostic artwork animations and animated overlay layers.
- Rebuilt the six diagnostic visuals as static Silverstone icons centred in one consistent scan-area box.
- Correct icon mapping: Aero = full Pit Perfect garage/fan, Stability = Stunt Driving, Power = Propulsion speedometer, Control = Guidance steering wheel, Traction = Control skidding car, Response = cone diagram.
- Corrected icon scale compensation so the source SVG whitespace does not make some icons appear much smaller than others.

# Pass 7.38.30 — MC02 Silverstone icon diagnostic suite

- Rebuilt all six Velocity Vault diagnostic visuals around one Silverstone-style icon system instead of six unrelated custom mini-diagrams.
- Aero now uses an isolated fan from the supplied Pit Perfect asset; the fan stays cyan and the airflow turns green when captured.
- Stability now uses the supplied Stunt Driving asset; it rocks during scanning, settles level, and the main icon turns green on capture.
- Power now uses a No Speed Limits-style icon; the icon stays cyan and only the performance trace turns green.
- Control now uses the existing Silverstone steering/control icon; the response arc and marker turn green while the wheel remains cyan.
- Traction now uses a drift-car/Caterham-style silhouette; only the four tyres turn green on capture.
- Response now uses the existing Silverstone cone/response diagram; the target turns green while the through-line remains cyan.
- Objective copy is now `Capture racing performance data to calibrate Santa-1’s recovery systems.`
- Card instruction remains `TAP TO SCAN`; final CTA remains `COMPLETE SCAN`.
- Mission Complete outcome is now `The racing performance data has been captured and is ready to support Santa-1’s recovery systems.`
- Scan timings, hit behaviour, card grid and CTA mechanics are unchanged.
